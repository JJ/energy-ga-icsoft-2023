...
\end{abstract}
% When mentioning the layers maybe include also the hardware layer, because many green computing techniques are implemented at that level. - Mario
% In the final sentence of the abstrast may use develop or create intead of come up with. - Mario
...
The experiment setup will match the one used in \cite{DBLP:conf/icsoft/GuervosGC23}, using the same tools for energy profiling ({\sf pinpoint}) as well as Perl scripts to run the experiments and process the results. % Mayba add some justification of why we are using the same tools. Resutls will be more comparable? - Mario
...
% Several data structures used in the implementation of EAs will be checked for: the default string, arrays of Boolean values and bit sets. - Mario
...
\item We will check the behavior of these on two different platforms, Linux (with AMD) and macOS (with M1).
% If Mac is the operating system, it is now refered to as macOS. - Mario
\end{itemize}
...
Figure \ref{fig:evostar.zig.gen} represents energy consumption, as well as time taken, for the different configurations taken as baseline. The first thing to notice, in the upper right corner, is that using a bitset in zig will not represent any energy saving, with a big difference in time as well as energy consumption; this is why it has not even been tested for the Mac. Second element that stands out is that consumption as well as time is very different for the Mac, which is an entry-level laptop computer, and the AMD desktop computer. This is probably expected, but the fact is that the MacBook Air, which is a generation behind current commercial offerings, takes 25\% of the time and 10\% of the energy to do the same amount of work. While the increase in energy consumption is quite steep in the AMD architecture used by the desktop computer, that is not the case for the Mac, with a very small increase from the smallest to the largest size.
