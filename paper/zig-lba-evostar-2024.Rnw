...
By default, all programs will be generated using the {\tt .ReleaseFast} compile option, that optimizes performance, but also energy consumption. We will be using the {\tt page_allocator} that allocates memory in the heap. This is an efficient allocator, but also the default choice.
