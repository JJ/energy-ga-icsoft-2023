...
The interest in greener computing has grown in the last decades, in part fueled by the Millennium Development Goals\footnote{Described in \url{https://www.un.org/millenniumgoals/bkgd.shtml}, for instance} that advocate for a lower carbon footprint in human activity and in part due to the fact that performance improvements brought by faster hardware are not coming with the same speed as they used to \cite{theis2017end}.
