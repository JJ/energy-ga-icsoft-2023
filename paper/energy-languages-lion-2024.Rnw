...
% We need a connection between the next two sencences? We are interested in EA/IA
% ...
% and there are many ways to lower the corbon footprint. 
%
% We could add something like: The IA field has cought the attention of many
% players but it is a very resource intensive task, so reducing the carbon
% footprint will have a global impact, but ...there are many ways.. 
...
\item {\sf pinpoint} does not have a version, 
\end{itemize}
