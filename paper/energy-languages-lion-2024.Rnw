...
% with less amount of energy? minimal implies the less possible, I don't know if this is the goal - M
% Changed to lesser - JJ
...
% And one of them... is hard to read, I had to go back and remember we are talking of choices 
% Maybe we need to remind the reader, And one of these choices is the programming language itself - M
% Changed - JJ
...
% Please consider this version (or a variant) to improve the paragraph below. It is a bit hard to follow - M 
...
% Is not clear what is a 'fundamental building block' or 'bigger span of the runtime' - M 
...
The trends need to be pondered too. In general, {\sf bun} is going to take longer and consume more energy than the rest for every size; Kotlin is the faster and boasts the lower consumption of energy, over all for smaller sizes. {\sf zig}, on the other hand, is faster than {\sf bun}, and its speed is quite consistent; not so for consumption of energy, which can go from very low, even less than Kotlin in some experiments (and virtually zero), to relatively high, with a maximum closer to the average for {\sf bun}.
...
combined.ops %>% select( language, size, mean.pkg, sd.pkg, ops.joules ) %>% kable( digits = 2, col.names = c("Language","Size","PKG average","PKG SD", "Ops/joule average"), caption = "Average operatios per Joule in the combined ops experimentfor the three languages" )
@
...
We need to check the speed and consumption of the selected fitness function, HIFF, for the three languages, so we can have a more complete picture of the energy profile for the three languages. This is a heavy-weight function, involving function calls in the order of one thousand. Even if it is going to take much more time than generating the chromosomes, we will still substract the time needed for that operation, simply to compare different things uniformly.
...
hiff %>% select( language, size, mean.pkg, sd.pkg, ops.joules ) %>% kable( digits = 2, col.names = c("Language","Size","PKG average","PKG SD", "Ops/joule average"), caption = "Average operatios per Joule in the HIFF experiment for the three languages" )
@

What we see in Table \ref{tab:hiff.joules} is the number of operations per Joule all three languages are able to perform.

\section{Discussion, conclusion and future work}
\label{sec:conclusions}

Implementing an algorithm implies making choices at different levels of abstraction, from the family of languages you will use to the specific language meeting your needs. Settling for languages at a certain level of abstraction will help you to stop making decisions at that level, removing the need to look further for alternatives, thus saving precious time in the computational experiment life cycle. % Conclude which languages are better

In this work, we compared the performance regarding power consumption of three languages, JavaScript, Kotlin, and Zig, when implementing evolutionary algorithms. We have used two workloads, one with a combination of genetic operators and the OneMax fitness function, and another with the HIFF fitness function. We have found that Kotlin is the most energy-efficient language, followed by Zig, and finally JavaScript (bun). This is consistent with previous work, where Kotlin was found to be the most efficient language in terms of energy consumption \cite{PEREIRA2021102609}. However, it is important to note that the differences between the languages are not as significant as in this previous work, where the difference between the most and the least efficient language was of two orders of magnitude. In this work, the difference is of one order of magnitude at most. This is probably due to the fact that we are using more complex workloads, which involve more function calls and memory management, and the fact that we are using a different measuring tool, which is more accurate than the one used before.

In future work, we plan to extend this study to other languages and workloads. On the algorithmic side, another venue of research is to analyze the energy consumption of languages implementing different models of concurrency, implemnted either by language constructs or through specialized virtual machines. We also plan to analyze the energy consumption of different hardware platforms (including the cloud), and to develop a tool to help researchers and developers to choose the most energy-efficient language for their needs.
