...
We need to check the speed and consumption of the selected fitness function, HIFF, for the three languages, so have a more complete picture of the energy profile of the three languages. This is a heavy-weight function, involving function calls of the order of one thousand. Even if it is going to take much more time than generating the chromosomes, we will still substract the time needed for that operation, simply to compare different things uniformly.
...
The results for time and energy consumption are represented in Figure \ref{fig:lion.hiff}. We can already observe that the consumption is more than one order of magnitude higher than before, and that since the number of function calls are dependent on the chromosome size, grows more or less linearly with that size. We can also observe that, as usual, {\sf bun} is the slowest and the one that consumes the most energy.

However, the situation with {\sf zig} and Kotlin is different. {\sf Kotlin} is in most cases slightly slower, but also more energy-saving, thus more {\em green} than  {\sf zig}. We represent a boxplot comparing them in Figure \ref{fig:lion.hiff.boxplot}.

<<lion.hiff.boxplot, echo=F, message=F, warning=F, fig.pos="h!tb", fig.height=4, fig.cap="Boxplot of the energy consumption of the HIFF fitness function for 40K chromosomes for the Kotlin and {\\protect\\sf zig}">>=
...
Differences are significant for all three languages, for all sizes involved; in the case of the biggest size, there is indeed a considerable difference, with Kotlin spending on average less than 1500 Joules.
