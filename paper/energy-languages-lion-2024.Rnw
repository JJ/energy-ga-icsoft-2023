...
All programs are run through a Perl script that captures and processes output, generating CSV data files that are committed to this repository, and also available under a free license. Instructions to compile and run it are also included in the repository; in general, all that is needed to make the results of this paper reproducible. 
