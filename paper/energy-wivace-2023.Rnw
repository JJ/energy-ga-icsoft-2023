...
<<wivace.crossover, echo=F, fig.height=4, fig.pos="h!tb", fig.cap="PKG consumption for the crossover and the three different virtual machines, shown as a boxplotPlease note this is the same data that was labelled May in Figure \\protect\\ref{fig:crossover}.\\protect\\label{fig:xover:vms}">>=
