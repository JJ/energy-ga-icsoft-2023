...
Green computing is a methodology for saving energy in the implementation of
  algorithms, and it is a problem with many moving parts. In environments where
  ...
  enough choice. In this paper, we will focus on JavaScript runtimes for
  ...
  has been used repeatedly to implement all kind of evolutionary algorithms
  almost since its inception. After settling the tools and essential
  methodology previously, in this paper we will focus on the importance of
  measuring different versions of the same runtimes, as well as extending the
  EA operators that will be measured. We would also like to remark the
  importance of testing the operators in different architectures, in order to
...
\item Test new versions of the virtual machines considering energy consumption, since in general these evolve towards more efficient operation; at any rate, the balance of results might change in these new versions in unexpected ways.
...
\item A more complete operation of an evolutionary algorithm will be tested: the ubiquitous mutation will also be tested, to see what kind of power consumption profile it adds.
\end{itemize}
...
In the area of evolutionary algorithms, initial energy efficiency estimations
included their behaviour in different platforms \cite{10.1007/978-3-319-45823-6_51}, its interaction with cloud services \cite{maryam2018evolutionary} and how it affected 
...
analysing how certain metaheuristics parameters have an impact on
energy consumption.  Díaz-Álvarez et al. \cite{diaz2022population}
studies how the populations size of EAs
influences power consumption. In an earlier work, centred on genetic
...
are available in this case, since it is a processor with a different architecture.

A Perl script was created to run the experiments; it executed the scripts and
collected results by parsing the standard output and putting it in a CSV format
that would allow examination of the experiments. As indicated in
\cite{js:energy}, consumption for a no-op task run in the same environment was
subtracted from the reading taken, before computing the average in this script.
...
What we will do is, repeat the setup in the initial exploration, 
...
and analysed in this paper, are released with a free license (along with this
...
The scenario that is shown in Figure \ref{fig:crossover}, representing the
performance for crossover, has some similarities with the one shown in Figure
\ref{fig:onemax}, showing again the strange performance correlations for the
{\sf deno} VM. In the other two cases, there is no great difference, it can even be slightly worse, although the difference does not seem to be significant.
...
%Rewrite
The figure shows how energy consumption grows less than linearly for {\sf bun}; this is a change with respect to \cite{js:energy}, when it was flat.
...
copy operations, its behaviour is quite similar to that shown in Figure \ref{fig:xover:vms}. {\sf deno} decreases with size, {\sf bun} increases very slowly, {\sf node} faster to the point that it spends three times more energy than {\sf bun} for the biggest size.
...
Results of these experiments for tne OneMax fitness function, shown in Figure
\ref{fig:onemax:intel}, first allow us to check the influence on the overall
...
\cite{js:energy}. The average time needed to find the solution, however, is higher, although not by an order of magnitude.
...
While in our previous paper, \cite{js:energy}, the measurements showed clearly
...
have an Intel processor and chipset. % or Apple Silicon :) - M 
