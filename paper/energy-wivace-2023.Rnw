...
<<r intel.onemax, echo=F, fig.show="hold", out.width="50%", fig.height=5, fig.cap="Cores consumption for the onemax fitness function and the three different virtual machines, shown as a boxplot; cores component (left) and RAM component (right). Please observe that the scales in the y axes are different.\\protect\\label{fig:onemax:intel}">>=
...
ggplot(onemax.intel, aes(x=size,y=cores))+geom_boxplot(aes(fill=VM))+ylim(0, NA)+theme_tufte()
ggplot(onemax.intel, aes(x=size,y=RAM))+geom_boxplot(aes(fill=VM))+ylim(0, NA)+theme_tufte()
