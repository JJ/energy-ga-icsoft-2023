...
algorithms are compared with respect their power efficiency. Machine learning/AI applications \cite{verdecchia2023systematic} have been studied from the green perspective more strongly since 2020, mainly using experimental studies, establishing it as the best practice, which we will follow also in this paper.

In the area of evolutionary algorithms, initial energy efficiency estimations included its interaction with cloud services \cite{maryam2018evolutionary} and how it affected 
genetic programming (GP) \cite{diaz2018fuzzy}; bug since then, and due to the fact that metaheuristics are so extensively used
...
This points to several best practices in the area: first, evaluate separately different operators and fitness functions, and second, perform experiments on different devices. After the initial exploration and establishment of technology in \cite{js:energy}, this paper will also test the mutation operator, as well as carry out tests in different computers with different architecture.
