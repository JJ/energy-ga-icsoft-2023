...
ggplot(combined.ops.zig, aes(x=size, y=PKG, color="Baseline"))+geom_boxplot(notch=T)+geom_point()+geom_boxplot(data=combined.ops.bool, aes(x=size, y=PKG, color="Boolean"))+geom_point(data=combined.ops.bool, aes(x=size, y=PKG, color="Boolean"))+geom_boxplot(data=combined.ops.fast, aes(x=size, y=PKG, color="ReleaseFast"))+geom_point(data=combined.ops.fast, aes(x=size, y=PKG, color="ReleaseFast"))+
  labs(title="Running time and PKG energy consumption, combined operations.",
       x="Chromosome size", y="PKG (Joules)") +
  theme_minimal()
@

This case is relatively similar to the one shown in Figure \ref{fig:lion.zig.setup}, even if now the majority of the operations are purely based in memory and CPU. Using a more efficient data structure, a Boolean array instead of a string, already reduces consumption almost by half; since in this case allocation is not used extensively, this proves that the majority of the improvement is due to the data structure and not the allocator used. We again obtain a 90\% reduction in energy consumption by generating the {\em fast} binary.
