...
as well as the actvivity of P- and E- cores. As in the case of the RAPL
...
What is remarkable, in this case, is also that the laptop is {\em faster}, with the workload taking on average \Sexpr{onemax.avg.time.mac$average[1]} seconds on the MacAir for OneMax with 1024 bits, and  \Sexpr{onemax.avg.time$average[1]} in the other case; all across the board, the MacAir is faster.

\section{Conclusions}
\label{sec:conc}

In this paper, we set out to study the energy efficiency of different JavaScript
interpreters and architectures in EA workloads, by studying how short scripts, that are extensively
...
measure per-process consumption; we have also adopted a multi-platform tool, \pinp, that is
...
operations can be assessed, discarding noise produced by the implementation of
a complete algorithm; that is, a whole program that includes different operations,
applied in different proportions, that might cancel each other. Testing short
...
During the exploratory data analysis, we have established that, in Linux and MacOS
...
interpreter and computer architecture should be used if our objective is to consume the minimum amount of
energy; the experiments have reliably confirmed {\sf bun} to be that tool across all architectures. It not
only consumes less for all the range of chromosome sizes used; it also takes
...
It has recently turned to version 1.0, so it is definitely our pick for this kind of workloads.

If that is an issue, {\sf deno} might be a good alternative. Except in very 
...
{\sf node}. As an inconvenience, it needs minor modifications for the code to run, at least
if you need core or other kinds of external libraries; its core library modules
are different to those used in {\sf node/bun}, although that need not be a
disadvantage per se, since it is not complicated to design algorithmic code that is totally interpreter-independent.

The previous two points imply that, energy-wise (or even performance-wise), there are no good reasons to
use {\sf node.js} for running EAs. Except if the business logic
uses specific, early-adoption, or some features that, for some reason, do not
work with {\sf bun} yet, we would advise anyone to keep using {\sf bun} for this
kind of workloads.

By testing different versions of the interpreters, we can affirm also that the advantage of {\sf bun} over the other interpreter is consistent in time; in some cases there are dramatic reductions in consumption, 
...
the machine design level. Different computers with different cooling
strategies, passive or active, will spend energy differently and
interact in non-trivial ways with workloads and interpreters. Even an architecture, the Apple Silicon M1, in a non-professional laptop, can outperform a professional desktop computer, and consume less energy in the process. Since in general researchers have different kinds of computers available, our measurements here indicate that, whenever possible, the Apple Silicon M1 architecture should be preferred.
...
could be an interesting future line of work; additionally to GPUs, Apple includes what is called the Apple Neural Engine. Although this is geared mainly towards tensor processing, implementing evolutionary algorithms to be able to use this kind of hardware would be a challenge, but it would be interesting to see how it performs and how it influences the energy profile.

Although mutation is not examined as a micro-operation, it will probably not have much influence; however, other operations like selection, with more complexity, might require energy expenses that scale in a wholly different way. In general, building up a more general instance of a single generation in an evolutionary algorithm will give us a wider perspective on energy profiles, without losing sight of what are the individual operations that make it up and the specific energy costs of each of them. At the same time, in this paper we have experimented with a single data structure for representation of the chromosomes; current EAs use any kind of data structures, and they will have different energy profiles. Fixing other parameters (like interpreters and architecture) and working on them will also give us a path for making evolutionary algorithms greener.
