...
operations can be pinpointed, discarding noise produced by the implementation of
...
energy; the experiments have reliably confirmed {\sf bun} to be that tool. It not
...
interact in non-trivial ways with workloads and interpreters. This will also
open interesting and new lines of research seeking the best software/hardware
combination to run EAs in a green way.
